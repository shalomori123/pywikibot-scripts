...
mefarshim = ['ילקוט שמעוני', 'מנחת שי', 'בעל הטורים', 'נחמיאש']
...
for mefaresh in mefarshim:
	for page in PrefixingPageGenerator(mefaresh + ' על '):
		if re.fullmatch(mefaresh + ' על (בראשית|שמות|ויקרא|במדבר|דברים|יהושע|שופטים|שמואל א|שמואל ב|מלכים א|מלכים ב|ישעיהו|ירמיהו|יחזקאל|תרי עשר|הושע|יואל|עמוס|עובדיה|יונה|מיכה|נחום|חבקוק|צפניה|חגי|זכריה|מלאכי|תהלים|משלי|איוב|שיר השירים|רות|אסתר|קהלת|איכה|דניאל|עזרא|נחמיה|דברי הימים א|דברי הימים ב)( ק?[ט-צ]?[א-י]?){2}', page.title()):
			edit_page.text += ' |\n' + str(page)
			print(page, 'added')
			counter += 1
			if not counter % 100:
				edit_page.save(summary='‏דפי מפרש על פסוק שאינם במרחב קטע')
edit_page.save(summary='‏דפי מפרש על פסוק שאינם במרחב קטע')
