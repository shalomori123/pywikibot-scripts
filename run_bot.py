...
#run(call + ['replace', '\{\{(מ"מ|ממ)\|([^\}]+)\|ק=\{\{שם הדף\}\}\}\}', '{{הפ|\\2}}', '-regex', '-transcludes:מ"מ', '-usercontribs:ShalomOrobot', '-intersect', '-summary:החלפת תבנית מ"מ לתבנית הפ'])
...
#run(call + [main_user,pedia,pt, 'listpages', draft.replace('-page', '-put'), '-format:2', '-overwrite', '-catr:יחסים בילטרליים לפי מדינה', '-titleregex:^יחסי ([א-ת]+ [א-ת ]+[^ ]–[^ ][א-ת ]+|[א-ת ]+[^ ]–[^ ][א-ת]+ [א-ת ]+|[א-ת]+ – [א-ת]+)$'])
...
#run(call + ['add_text', '-prefixindex:חבל נחלתו ', '-text:[[קטגוריה:חבל נחלתו {{ס:החלף|{{ס:החלף|{{ס:שם הדף}}|חבל נחלתו |}}| |{{ס:!}}}}]]', '-grepnot:\[\[קטגוריה:חבל נחלתו', '-summary:קטגוריה'])
#run(call + ['replace', *visual_editor, '-page:שיחות הר"ן/שלם'])
#run(call + ['sources_bot', '-page:שיחות הר"ן/שלם'])
#run(call + ['replace', '(?<!small>)(?<!\{\{קטן\|)(\(...+?\))', '{{קטן|\\1}}', '-regex', '-page:שיחות הר"ן/שלם', '-summary:הקטנת כל סוגריים, לבקשת משתמש:ראובן פרוס'])
...
#run(call + ['replace', "גמ['’‘׳]:", "\n<big>'''גמ’:'''</big>", '(?:\{\{(?:תנא|אמורא)-ירו"מ\|)?(רבי בנאה|רבן גמליאל ברבי|רבי חנינא בר חמא|יהודה בן רבי חייא|רבי חייא|רבי חייא רובה|רבי חייא רבה|רבי יהושוע בן לוי|רבי יהושע בן לוי|רבי הושעיה|רבי הושעיה רבה|רבי הושעיה רובה|רבי אושעיה בר חמא|פלימו|בר קפרא|אייבו|רבי יצחק בר אבדימי|רבי שמעון בן יהוצדק|רבי שמעון ברבי|רבן גמליאל ברבי|רבי אמי בן אסי|רבי שילא)(?:\}\})?', '{{דור המעבר-ירו"מ|\\1}}', '-regex', '-prefixindex:ביאור:ירושלמי מאיר/', '-summary:הדגשת גמ\', תבנית דור המעבר'])
#run(call + ['replace', '(?<!\{\{תנא-ירו"מ\|)בית שמאי', '{{תנא-ירו"מ|בית שמאי}}', '(?<!\{\{תנא-ירו"מ\|)בית הלל', '{{תנא-ירו"מ|בית הלל}}', '-regex', '-prefixindex:ביאור:ירושלמי מאיר/', '-summary:תיקוני שגיאות בתנאים ואמוראים'])
#run(call + ['replace', '{{תנא|', '{{תנא-ירו"מ|', '{{אמורא|', '{{אמורא-ירו"מ|', '{{אמורה|', '{{אמורא-ירו"מ|', '{{תנא-ירו"מ|הלל}}', 'הלל', '([[{{אמורא-ירו"מ|שמואל}}', '([[שמואל', '-prefixindex:ביאור:ירושלמי מאיר/', '-summary:תיקוני שגיאות בתנאים ואמוראים'])
...
#run(call + ['replace', '\{\{תנא\| *', '{{תנא-ירו"מ|', '\{\{אמורא\| *', '{{אמורא-ירו"מ|', '\{\{ירושלמי מאיר/קישורים\|', '{{תחילת עמוד-ירו"מ|', '\[דף .*? עמוד [אב]\] *', '', "('''עין משפט[\s\S]*?)(\n===דף)", '{{תיבה נגללת}}\\1{{סוף}}\\2', '\{\{ירושלמי מאיר/הלכה\|(.*?)\|([ט-כ][א-ט]?|[א-ט])\|([ט-כ][א-ט]?|[א-ט])\}\}', "==[[ירושלמי \\1 \\2 \\3|'''ירושלמי \\1, פרק \\2, הלכה \\3''']]==", '===דף ([ט-צ][א-ט]?|[א-ט])===', '===-----------------------------------{{צבע גופן|אדום|דף \\1}}===', '-regex', '-prefixindex:ביאור:ירושלמי מאיר/', '-summary:תבנית תנא ואמורא, תיבה נגללת לעין משפט, עיצוב כותרות'])
